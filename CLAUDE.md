...
## v2.3 (돌아오는 길)
"운세나 설정에 들어가면 메인으로 돌아올 수가 없다"는 신고. 탭바는 렌더되고 있었는데 **화면 밖에 있었다.**

### 원인: CSS 순서
`style.css`에서 데스크톱용 `.tabbar` 규칙이 **기본 `.tabbar` 규칙보다 앞**에 있었다(75행 vs 445행).
특이도가 같으므로 뒤에 오는 기본 규칙의 `left: 0; right: 0`이 이기고, 앞 규칙의 `transform: translateX(-50%)`는 덮이지 않아 그대로 남았다.
→ 900px 이상 화면에서 760px짜리 탭바가 **왼쪽 끝에 붙은 채 380px 왼쪽으로 끌려나가** 절반이 화면 밖으로 사라졌다.

**교훈**: 미디어 쿼리는 자기가 덮으려는 기본 규칙 **뒤에** 놓아야 한다. 파일 위쪽에 몰아두면 이렇게 조용히 반쪽만 적용된다.
고친 뒤 규칙도 자기완결적으로: `left: 50%; right: auto; transform: translateX(-50%); width: 760px`.

### 돌아오는 길을 셋으로
탭바 하나에만 의존하면 그게 깨졌을 때 갇힌다.
- **로고가 홈 버튼** — 상단바 `LOOP` 마크가 `<button class="mark homebtn">`, 누르면 오늘 탭
- **`← 오늘` 버튼** — 오늘 탭이 아닐 때만 상단바에 나타난다
- **실행 모드**에도 `닫기` 옆에 `오늘 계획` 버튼 (실행 모드는 일부러 탭바를 숨기므로 나갈 길이 하나뿐이었다)

### 테스트
`nav.test.js` 34개 — 네 탭 전부에서 탭바·탭 4개·활성 표시, 로고/`← 오늘`/실행 모드에서 한 번에 복귀, 그리고 **CSS 파일을 직접 읽어 데스크톱 규칙이 기본 규칙보다 뒤에 있는지 검사**한다. 순서가 다시 뒤집히면 테스트가 잡는다.
